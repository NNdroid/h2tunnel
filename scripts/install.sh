  ...
  if [ ! -f "${config_file}" ]; then
    if [ "${mode}" == "client" ]; then
      if [ -f "./config.client.json" ]; then
        ...
        curl -fsSL "https://raw.githubusercontent.com/${GITHUB_REPO}/main/config.client.json" -o "${config_file}" || true
      fi
    else
      if [ -f "./config.server.json" ]; then
        ...
        curl -fsSL "https://raw.githubusercontent.com/${GITHUB_REPO}/main/config.server.json" -o "${config_file}" || true
      fi
    fi
...
install_systemd() {
  ...
  local config_file
  config_file=$(get_config_file "${mode}")
  local desc="HTTP/2, HTTP/3, WebTransport & MASQUE Server"
  if [ "${mode}" == "client" ]; then
    desc="HTTP/2, HTTP/3, WebTransport & MASQUE Client"
  fi
