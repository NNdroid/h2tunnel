...
// attachUDPSession injects the udpSession data plane (h2/h2c/h3/grpc/masque).
...
// closeFunc adapts a void close function into an io.Closer (idempotency is the
// wrapped side's responsibility).
type closeFunc func()
...
func (c *virtualPacketConn) fail(err error) {
	...
	if c.onDead != nil {
		go c.onDead(c.terminalErr)
	}
}

func (c *virtualPacketConn) safeDead(err error) {
	if c.onDead != nil {
		c.onDead(err)
	}
}
