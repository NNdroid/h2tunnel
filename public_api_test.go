...
func TestPublicAPIThroughCDN(t *testing.T) {
	...
	client, safeHeaders := newPublicAPICDNEnvironmentWithPadding(t, 2*time.Millisecond, padding)
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	conn, err := client.DialContext(ctx, h2tunnel.NetworkTCP, "echo")
	if err != nil {
...
func newPublicAPICDNEnvironment(tb testing.TB, proxyWriteDelay time.Duration) (*h2tunnel.Client, *atomic.Bool) {
	return newPublicAPICDNEnvironmentWithPadding(tb, proxyWriteDelay, h2tunnel.PaddingTuning{})
}

func newPublicAPICDNEnvironmentWithPadding(tb testing.TB, proxyWriteDelay time.Duration, padding h2tunnel.PaddingTuning) (*h2tunnel.Client, *atomic.Bool) {
	tb.Helper()
	echoListener, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		tb.Fatal(err)
