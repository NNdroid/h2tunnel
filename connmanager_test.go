...
func TestConnManagerPrimaryBackupCounts(t *testing.T) {
	...
	waitCount(t, func() bool { return m.PrimaryCount() == 1 }, 30*time.Second, "主连接应拨 1 条")
	waitCount(t, func() bool { return m.BackupCount() == 1 }, 30*time.Second, "备用应在 establish 间隔后拨 1 条")
...
func TestConnManagerTypeSharding(t *testing.T) {
	...
	waitCount(t, func() bool { return m.PrimaryCount() == 2 }, 30*time.Second, "应拨 2 条主")
	...
	waitCount(t, func() bool { return m.BackupCount() == 1 }, 30*time.Second, "备用应拨 1 条")
...
func TestConnManagerSwitchoverReplenish(t *testing.T) {
	...
	// 等主备就绪
	waitCount(t, func() bool { return m.PrimaryCount() == 1 && m.BackupCount() == 1 }, 30*time.Second, "主备就绪")

	// 主连接阵亡 → 触发升级+补位
	m.FailPrimary("default")

	// 升级后主数量应恢复 1，且备用补足 1
	waitCount(t, func() bool { return m.PrimaryCount() == 1 && m.BackupCount() == 1 }, 30*time.Second, "主断后应升级备用并补足数量")
...
func TestConnManagerBackupKeepaliveFailure(t *testing.T) {
	...
	waitCount(t, func() bool { return m.BackupCount() == 1 }, 30*time.Second, "备用就绪")
	...
	// 失效被移除 → 补位回到 1 条
	waitCount(t, func() bool { return m.BackupCount() == 1 }, 30*time.Second, "失效备用应被移除并补足")
	// 主连接不受影响
	if m.PrimaryCount() != 1 {
...
func TestConnManagerDialIntervalThrottle(t *testing.T) {
	...
	waitCount(t, func() bool { return m.PrimaryCount() == 1 }, 30*time.Second, "主就绪")
...
func TestConnManagerEstablishInterval(t *testing.T) {
	...
	// 主应立即出现
	waitCount(t, func() bool { return m.PrimaryCount() == 1 }, 30*time.Second, "主立即建立")
	...
	waitCount(t, func() bool { return m.BackupCount() == 1 }, 30*time.Second, "备在间隔后建立")
