...
// TestPublicAPIDialErrorSentinels verifies tunnel-layer HTTP rejections map to
// public sentinels: 407/401 → ErrUnauthenticated, 403 → ErrForbidden. Embedders
// use this to tell "swap the token" from "swap the target" without parsing
// error strings.
func TestPublicAPIDialErrorSentinels(t *testing.T) {
	...
	// Closed registry: only "echo" is registered, dialing "missing" is guaranteed 403.
	...
	// No credentials → server 407 → ErrUnauthenticated.
	unauthed := newClient("")
	...
	// Correct credentials but unregistered target → server 403 → ErrForbidden.
